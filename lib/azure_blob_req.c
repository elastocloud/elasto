		    ...
		    struct op **_op)
{
	...
	ret = az_blob_ebo_init(AOP_ACC_KEYS_GET, &ebo);
	if (ret < 0) {
		goto err_out;
	}
	op = &ebo->op;
		...
		struct op **_op)
{
	...
	ret = az_blob_ebo_init(AOP_ACC_LIST, &ebo);
	if (ret < 0) {
		goto err_out;
	}
	op = &ebo->op;
		  ...
		  struct op **_op)
{
	...
	ret = az_blob_ebo_init(AOP_ACC_CREATE, &ebo);
	if (ret < 0) {
		goto err_out;
	}
	op = &ebo->op;
	       ...
	       struct op **_op)
{
	...
	ret = az_blob_ebo_init(AOP_ACC_DEL, &ebo);
	if (ret < 0) {
		goto err_out;
	}
	op = &ebo->op;
		  ...
		  struct op **_op)
{
	...
	ret = az_blob_ebo_init(AOP_STATUS_GET, &ebo);
	if (ret < 0) {
		goto err_out;
	}
	op = &ebo->op;
